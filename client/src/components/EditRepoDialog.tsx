...
interface EditRepoDialogProps {
  ...
  onSubmit: (id: number, useGlobalCron: boolean, cronExpression: string, localVersion?: string) => Promise<void>;
  globalCron: string;
}

export function EditRepoDialog({ open, onOpenChange, repo, onSubmit, globalCron }: EditRepoDialogProps) {
  const { t } = useI18nStore();
  const [loading, setLoading] = useState(false);
  const [useGlobalCron, setUseGlobalCron] = useState(repo.useGlobalCron);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(repo.id, useGlobalCron, useGlobalCron ? globalCron : cronExpression, localVersion || undefined);
      onOpenChange(false);
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                id="edit-useGlobalCron"
                checked={useGlobalCron}
                onCheckedChange={(checked) => setUseGlobalCron(checked === true)}
              />
              <Label htmlFor="edit-useGlobalCron" className="cursor-pointer">{t('editRepoDialog.useGlobalCron')}</Label>
            </div>
            {!useGlobalCron && (
              <div className="space-y-2">
